...
class PluginRepository:
    ...
    def get_exact(
        ...
        Args:
            resource_ids: An iterable of plugin resource IDs
            deletion_policy: Whether to look at deleted plugins,
                non-deleted plugins, or all plugins

        Returns:
            A list of Plugin objects matching the given IDs
    ...
    def get_one_snapshot(
        ...
        """
        Get a specific plugin snapshot given the resource snapshot ID; require
    ...
    def get_by_filters_paged(
        ...
        Args:
            ...
            filters: A list of parsed filters to apply
    ...
    def _assert_plugin_file_name_available(
        ...
        if exclude_resource_id is not None:
            stmt = stmt.where(PluginFile.resource_id != exclude_resource_id)
    ...
    def create_file(self, plugin_file: PluginFile, plugin_id: int | None = None):
        ...
        Args:
            ...
            plugin_id: The parent plugin's resource ID. Optional, can be derived
                from parents.
    ...
    def get_one_file(
        ...
        exactly one is found, or raise an exception. The file must be associated
        with the latest snapshot of the given plugin.

        Args:
            plugin_id: The parent plugin resource ID
            plugin_file_id: The plugin file resource ID
        ...
        """
        return utils.get_one_latest_child_snapshot(
            self.session,
            PluginFile,
            EntityType.PLUGIN_FILE,
            plugin_file_id,
    ...
    def get_one_plugin_plugin_file(
        ...
        plugin_file_snapshot_id: int,
    ) -> m.PluginPluginFile:
        """Get the association for this plugin and plugin file snapshot pair.

        Args:
            plugin_snapshot_id: The plugin snapshot ID.
            plugin_file_snapshot_id: The plugin file snapshot ID.

        Returns:
            The PluginPluginFile association.

        Raises:
            EntityRelationshipDoesNotExistError: If the plugin snapshot is not
                associated with the plugin file snapshot.
        """
        stmt = sa.select(m.PluginPluginFile).where(
            m.PluginPluginFile.plugin_file_resource_snapshot_id
            == plugin_file_snapshot_id,
            m.PluginPluginFile.plugin_resource_snapshot_id == plugin_snapshot_id,
        )
        plugin_plugin_file = self.session.scalar(stmt)
        ...
        return plugin_plugin_file

    def get_one_task(self, task_id: int) -> m.PluginTask:
        """Get a plugin task by task ID.
        ...
        Raises:
            PluginTaskDoesNotExistError: If the PluginTask is not found.
        """
        task = self.session.get(m.PluginTask, task_id)
    ...
    def add_plugin_tasks(
        ...
        """Add plugin tasks to a plugin file snapshot.

        Args:
            function_tasks: Function task definitions to add.
            artifact_tasks: Artifact task definitions to add.
            plugin_file: The plugin file snapshot that owns the tasks.
            types: Parameter type snapshots referenced by the task definitions.

        Raises:
            KeyError: If a task references a parameter type that is not present in
                ``types``.
        """
