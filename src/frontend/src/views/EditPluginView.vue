<template>
  <div class="row items-center justify-between">
    <PageTitle :title="ORIGINAL_PLUGIN?.name" resourceType="plugin" :deleted="plugin?.deleted" />
    <q-btn 
      v-if="!plugin?.deleted"
      color="negative"
      icon="sym_o_delete" 
  ...
  <q-banner v-if="plugin?.deleted" dense class="text-white bg-red q-mt-md">
    <template #avatar>
  ...
  >
    ...
    <q-card class="q-pa-md">
      <KeyValueTable :rows="pluginRows" :secondColumnFullWidth="true">
        <template #name="{ }">
          {{ name }}
          <q-btn v-if="!plugin?.deleted" icon="edit" round size="sm" color="primary" flat />
          <q-popup-edit
            v-model="name" 
            auto-save 
            v-slot="scope"
        ...
        <template #description="{ }">
          <div class="row items-center no-wrap">
            ...
            <q-btn v-if="!plugin?.deleted" icon="edit" round size="sm" color="primary" flat class="q-ml-xs" />
          </div>
