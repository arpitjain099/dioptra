    ...
    >
      ...
      <div class="col">
        ...
        <TableComponent
          ...
          style="margin-top: 0px"
          @edit="
            router.push(`/artifacts/${selectedUsedArtifact[0].id}?snapshotId=${selectedUsedArtifact[0].snapshotId}`)
          "
        />
      </div>
    </div>
...
async function loadAllMetricHistories() {
  metricsData.value = [];
  await getJobMetrics();
  const promises = metricNames.value.map((name) => getJobMetricHistory(route.params.id, name));
...
const filteredMetrics = computed(() => {
  ...
  return metricsData.value.filter((metric) => metric.name.toLowerCase().includes(filter.value.toLowerCase()));
});
...
const overviewRows = computed(() => [
  ...
  { label: "Experiment", slot: "experiment", props: { experiment: job.value?.experiment } },
  { label: "Entrypoint", slot: "entrypoint", props: { entrypoint: job.value?.entrypoint } },
  {
    label: "Queue",
    slot: "queue",
